...
function updateArea(e) {
    ...
    console.log("GET /polygon?geojson="+JSON.stringify(data));
...
map.on('load', function() {
    ...
    map.on('dblclick', 'unclustered-point', function (e) {
        ...
        console.log("GET /nearestpoi?longitude="+actualPosition.getLngLat().lng+"&latitude="+actualPosition.getLngLat().lat+"&distance="+distance*1000);
        jQuery.get(url+"/nearestpoi?longitude="+actualPosition.getLngLat().lng+"&latitude="+actualPosition.getLngLat().lat+"&distance="+distance*1000, function(response) {
            console.log(JSON.stringify(response, null, 2));
            map.getSource("poi").setData(response);
        });
    });
    ...
    jQuery.get(url+"/heatmapcountries", function(response) {
        console.log("GET /heatmapcountries");
        console.log(JSON.stringify(response, null, 2));
...
document.querySelector('#nearestcount').onclick = function() {
    ...
    console.log("GET /nearestcount?longitude"+actualPosition.getLngLat().lng+"&latitude"+actualPosition.getLngLat().lat+"&limit"+limit+getFilters());
    // myLayer.clearLayers();
...
document.querySelector('#nearest').onclick = function() {
    console.log("#nearest ");
    var distance = document.getElementById("txt_nearest").value;
    console.log("GET /nearest?longitude="+actualPosition.getLngLat().lng+"&latitude="+actualPosition.getLngLat().lat+"&distance="+distance*1000);
    // clearLayouts();
...
document.querySelector('#heatmapdistrictsbtn').onclick = function() {
    ...
    console.log("GET /heatmapdistricts")
    jQuery.get(url+"/heatmapdistricts", function(response) {
        console.log(JSON.stringify(response, null, 2));
        var heatMapCountries = response;
        console.log(heatMapCountries);
        var arrayLength = heatMapCountries.length;
        var max = heatMapCountries[3].count;
        for (var i = 0; i < arrayLength; i++) {
            ...
            console.log(heatMapCountries[i].name);
            console.log(heatMapCountries[i].count);
            console.log(url+"/boundarydistrictsname?name="+name+"&count="+count);
...
document.querySelector('#boundariesbtn').onclick = function() {
    ...
    console.log("GET /boundary?id="+id);
...
document.querySelector('#boundariesbtn2').onclick = function() {
    ...
    var comboBox = document.getElementById('boundaries');
    var id = comboBox.options[comboBox.selectedIndex].value;
    console.log("GET /boundarypoints?id="+id);
